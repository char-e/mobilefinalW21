...
public class FavouritesRepository {
    ...
    public Article save(Article article) {
        ...
        long id = db.insert(DatabaseHelper.TABLE_NAME2, null, row);
    ...
    public List<Article> findAll() {
        ...
        List<Article> favourites = new ArrayList<>(results.getCount());
        if (results.getCount() > 0) {
            while (results.moveToNext()) {
                favourites.add(new Article()
    ...
    public void delete(Article article) {
        db = helper.getWritableDatabase();
        db.delete(DatabaseHelper.TABLE_NAME2, "HEADLINE=?", new String[]{article.getHeadline()});
        db.close();
    }
